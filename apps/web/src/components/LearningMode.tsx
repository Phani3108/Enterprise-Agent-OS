...
export function LearningMode() {
    ...
    return (
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            {/* Header */}
            <div className="px-6 py-4 border-b border-slate-200 bg-gradient-to-r from-emerald-500/5 to-teal-500/5">
                <div className="flex items-center gap-3 mb-2">
                    <span className="text-xl">🎓</span>
                    <h2 className="text-base font-semibold text-slate-900">{topic.title}</h2>
                </div>
                <div className="flex items-center gap-3">
                    <span className={`text-[11px] px-2 py-0.5 rounded-full ${LEVEL_COLORS[topic.level]}`}>
                        {topic.level}
                    </span>
                    <span className="text-xs text-slate-400">⏱ {topic.estimatedTime}</span>
                    <span className="text-xs text-slate-400">📖 {topic.sections.length} sections</span>
                    <span className="text-xs text-slate-400">💻 {topic.codeExamples.length} examples</span>
                    <span className="text-xs text-slate-400">🏋️ {topic.exercises.length} exercises</span>
                </div>
            </div>
                    ...
                    <div className="space-y-6">
                        {topic.sections.map((section, idx) => (
                            <div key={idx} className="animate-fade-in" style={{ animationDelay: `${idx * 50}ms` }}>
                                ...
                                <p className="text-xs text-slate-500 leading-relaxed whitespace-pre-line pl-7">
                                    {section.content}
                                </p>
                                {section.diagram && (
                                    <pre className="mt-3 ml-7 p-3 rounded-lg bg-white text-[11px] text-accent font-mono leading-relaxed border border-accent/10">
                                        {section.diagram}
                                    </pre>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {topic.internalRefs.map((ref, idx) => (
                                    <a
                                        key={idx}
                                        href={ref.url}
                                        className="flex items-center gap-2 p-2 rounded-lg bg-white hover:bg-slate-50 transition-colors"
                                    >
                                        <span className="text-[11px] px-1.5 py-0.5 rounded font-mono uppercase bg-accent/10 text-accent">
                                            {ref.type}
                                        </span>
                                        <span className="text-xs text-slate-600">{ref.title}</span>
                                    </a>
                                ))}
                            </div>
                        </div>
                        ...
                        {topic.codeExamples.map((example, idx) => (
                            <div key={idx} className="rounded-lg border border-slate-200 overflow-hidden">
                                <div className="flex items-center justify-between px-4 py-2 bg-slate-50 border-b border-slate-200">
                                    <span className="text-xs font-medium text-slate-900">{example.title}</span>
                                    <div className="flex items-center gap-2">
                                        <span className="text-[11px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-500 font-mono">
                                            {example.language}
                                        </span>
                                        {example.runnable && (
                                            <button className="text-[11px] px-2 py-0.5 rounded bg-accent/20 text-accent hover:bg-accent/30 transition-colors">
                                                ▶ Run
                                            </button>
                                        )}
                                    </div>
                                </div>
                                <pre className="p-4 text-[11px] text-slate-600 font-mono leading-relaxed overflow-x-auto bg-white">
                                    {example.code}
                                </pre>
                            </div>
                        ))}
                ...
                {activeTab === 'exercises' && (
                    <div className="space-y-3">
                        {topic.exercises.map((exercise, idx) => (
                            <div
                                key={idx}
                                className="p-4 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 transition-colors cursor-pointer"
                            >
                                <div className="flex items-center gap-2 mb-2">
                                    <span className={`text-[11px] px-2 py-0.5 rounded-full border ${DIFFICULTY_COLORS[exercise.difficulty]}`}>
                                        {exercise.difficulty}
                                    </span>
                                    <h4 className="text-sm font-medium text-slate-900">{exercise.title}</h4>
                                </div>
