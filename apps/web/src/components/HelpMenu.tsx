    ...
    return (
        <div ref={menuRef} className="relative" data-tour="help-menu">
            <button
                ...
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs text-neutral-400 hover:text-white hover:bg-white/5 transition-colors"
            ...
            {open && (
                <div
                    className="absolute bottom-full left-0 mb-2 w-56 rounded-xl border border-white/10 shadow-2xl overflow-hidden animate-fade-in"
                    ...
                    <div className="p-1">
                        {items.map((item, i) => (
                            <button
                                key={i}
                                onClick={item.action}
                                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-neutral-300 hover:bg-white/5 hover:text-white transition-colors text-left"
                                role="menuitem"
                            >
                                <span>{item.icon}</span>
