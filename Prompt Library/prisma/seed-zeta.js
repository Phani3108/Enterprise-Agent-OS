...
async function main() {
    ...
    // Create admin user
    const password = await bcryptjs_1.default.hash("AgentOSAdmin2026!", 12);
    ...
    for (const promptData of platformPrompts) {
        ...
        // Connect tags
        if (tags.length > 0) {
            const tagIds = tags.map(tagSlug => tagMap.get(tagSlug)).filter(Boolean);
...
//# sourceMappingURL=seed-zeta.js.map