{{- define "common.deployment-springboot" }}
...
{{- end }}
{{- if .Values.containerLifecycleControl }}
  {{- if .Values.containerLifecycleControl.preStop }}
    {{- $preStop := .Values.containerLifecycleControl.preStop }}
    {{- if and $preStop.enabled (or $preStop.exec $preStop.httpGet) }}
          lifecycle:
            preStop:
              {{- if $preStop.exec }}
              exec:
{{ toYaml $preStop.exec | indent 16 }}
              {{- end }}
              {{- if $preStop.httpGet }}
              httpGet:
{{ toYaml $preStop.httpGet | indent 16 }}
              {{- end }}
    {{- end }}
  {{- end }}
{{- end }}
...
{{- end }}
          livenessProbe:
            httpGet:
              port: http
              path: {{ default "/status" .Values.service.healthCheckPath }}
              {{- if .Values.service.probeHttpHeaders }}
              httpHeaders:
                {{- toYaml .Values.service.probeHttpHeaders | trim | nindent 14 }}
              {{- end }}
            ...
            successThreshold: 1
            failureThreshold: {{ default 3 .Values.service.livenessFailureThreshold }}
            timeoutSeconds: {{ default 1 .Values.service.livenessTimeoutSeconds }}
          readinessProbe:
            httpGet:
              port: http
              path: {{ default "/status" .Values.service.healthCheckPath }}
            ...
            successThreshold: 1
            failureThreshold: {{ default 3 .Values.service.readinessFailureThreshold }}
            timeoutSeconds: {{ default 1 .Values.service.readinessTimeoutSeconds }}
          resources:
...
{{- if (.Values.topologySpreadConstraints).enabled }}
  {{- if or (.Values.topologySpreadConstraints.azspread).enabled (.Values.topologySpreadConstraints.nodespread).enabled .Values.topologySpreadConstraints.additionalConfig }}
