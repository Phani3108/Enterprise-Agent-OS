...
async function post(path: string, data: unknown = {}): Promise<{ status: number; body: Record<string, unknown> }> {
...
async function patch(path: string, data: unknown = {}): Promise<{ status: number; body: Record<string, unknown> }> {
    const res = await fetch(`${BASE}${path}`, {
...
async function put(path: string, data: unknown = {}): Promise<{ status: number; body: Record<string, unknown> }> {
    const res = await fetch(`${BASE}${path}`, {
...
async function testCore(): Promise<void> {
    ...
    await test('GET /api/skills returns skill catalog', async () => {
        ...
        assert((body.skills as unknown[]).length > 0, 'should have at least one skill');
    });
...
async function testPersonas(): Promise<void> {
    ...
    await test('GET /api/personas returns all personas', async () => {
        ...
        const personas = body.personas as { id: string }[];
    ...
    await test('GET /api/personas/:id returns a specific persona', async () => {
        ...
        const first = (list.personas as { id: string }[])[0];
...
async function testMarketplace(): Promise<void> {
    ...
    await test('POST /api/marketplace/skills creates a new skill', async () => {
        ...
        createdSkillId = (body.skill as { id: string }).id;
    });

    await test('GET /api/marketplace/skills/:id returns created skill', async () => {
        if (!createdSkillId) { console.log('     skip — no skill created'); return; }
        const { status, body } = await get(`/api/marketplace/skills/${createdSkillId}`);
        assertStatus(status, 200);
        assert((body.skill as { id: string }).id === createdSkillId, 'should return correct skill');
    });
...
async function testIntentEngine(): Promise<void> {
    console.log('\n── Intent Engine ─────────────────────────────────');
...
async function testPromptLibrary(): Promise<void> {
    console.log('\n── Prompt Library ────────────────────────────────');
...
async function testMemoryGraph(): Promise<void> {
    console.log('\n── Memory Graph ──────────────────────────────────');
...
async function testToolRegistry(): Promise<void> {
    ...
    await test('POST /api/tools/registry registers a new tool', async () => {
        ...
        registeredToolId = (body.tool as { id: string }).id;
    });

    await test('DELETE /api/tools/registry/:id removes a tool', async () => {
        if (!registeredToolId) { console.log('     skip — no tool registered'); return; }
        const { status, body } = await del(`/api/tools/registry/${registeredToolId}`);
        assertStatus(status, 200);
        assert((body as { deleted: string }).deleted === registeredToolId, 'should confirm deletion');
    });
}
...
async function testScheduler(): Promise<void> {
    ...
    await test('POST /api/scheduler/jobs creates a scheduled job', async () => {
        ...
        createdJobId = (body.job as { id: string }).id;
    });

    await test('GET /api/scheduler/jobs/:id returns job with logs', async () => {
        if (!createdJobId) { console.log('     skip — no job created'); return; }
    ...
    await test('POST /api/scheduler/jobs/:id/run executes job manually', async () => {
        ...
        const log = body.log as { status: string };
    ...
    await test('PATCH /api/scheduler/jobs/:id updates a job', async () => {
        ...
        assertStatus(status, 200);
        assert((body.job as { name: string }).name === 'Updated E2E Job', 'name should be updated');
    });
    ...
    await test('DELETE /api/scheduler/jobs/:id deletes a job', async () => {
        ...
        assertStatus(status, 200);
        assert((body as { deleted: string }).deleted === createdJobId, 'should confirm deletion');
    });
...
async function testBlog(): Promise<void> {
    ...
    await test('PATCH /api/blog/posts/:id updates a post', async () => {
        ...
        assertStatus(status, 200);
        assert((body.post as { title: string }).title === 'Updated E2E Test Post', 'title should be updated');
    });
    ...
    await test('POST /api/blog/posts/:id/publish publishes post to destinations', async () => {
        ...
        const publishResults = body.results as Array<{ destination: string; status: string }>;
        ...
        const publishedPost = body.post as { status: string; destinations: string[] };
    ...
    await test('POST /api/blog/posts/:id/like increments likes', async () => {
        ...
        assertStatus(status, 200);
        assert((body.post as { likeCount: number }).likeCount > 0, 'likeCount should be incremented');
    });
    ...
    await test('GET /api/blog/posts?status=draft filters by status', async () => {
        ...
        assertStatus(status, 200);
        const posts = body.posts as Array<{ status: string }>;
...
async function testForum(): Promise<void> {
    ...
    await test('GET /api/forum/threads?sort=hot sorts by hot', async () => {
        ...
        const threads = body.threads as Array<{ isPinned: boolean }>;
    ...
    await test('GET /api/forum/threads?sort=new sorts by newest', async () => {
        ...
        assertStatus(status, 200);
        const threads = body.threads as Array<{ createdAt: string; isPinned: boolean }>;
    ...
    await test('POST /api/forum/threads creates a new thread', async () => {
        ...
        const thread = body.thread as { id: string; upvotes: number };
    ...
    await test('GET /api/forum/threads/:id returns thread with comments', async () => {
        if (!createdThreadId) { console.log('     skip — no thread created'); return; }
    ...
    await test('POST /api/forum/threads/:id/comments adds a comment', async () => {
        ...
        createdCommentId = (body.comment as { id: string }).id;
    });
    ...
    await test('POST /api/forum/comments/:id/vote upvotes a comment', async () => {
        if (!createdCommentId) { console.log('     skip — no comment created'); return; }
...
async function testLicenses(): Promise<void> {
    console.log('\n── Corp IT / Licenses ────────────────────────────');
...
async function testAgentEvals(): Promise<void> {
    console.log('\n── Agent Evals ──────────────────────────────────');

    await test('GET /api/evals/dashboard returns dashboard data', async () => {
        const { status, body } = await get('/api/evals/dashboard');
        assertStatus(status, 200);
        assert(typeof (body as any).totalAgents === 'number', 'totalAgents should be a number');
        assert(typeof (body as any).avgHealthScore === 'number', 'avgHealthScore should be a number');
        assert(Array.isArray((body as any).agents), 'agents should be an array');
    });
    ...
    await test('GET /api/evals/agents/:id returns single agent', async () => {
        ...
        testAgentId = (listBody as any).agents[0].agentId;
        const { status, body } = await get(`/api/evals/agents/${testAgentId}`);
        assertStatus(status, 200);
        assert((body as any).agent.agentId === testAgentId, 'should match requested agent');
    });
    ...
    await test('PUT /api/evals/agents/:id updates config', async () => {
        ...
        assertStatus(status, 200);
        assert((body as any).agent.driftThreshold === 25, 'driftThreshold should be updated');
    });
    ...
    await test('GET /api/evals/agents/:id/drift returns drift history', async () => {
        ...
        assertStatus(status, 200);
        assert(Array.isArray((body as any).history), 'history should be array');
    });
    ...
    await test('POST /api/evals/agents/:id/baseline sets baseline', async () => {
        ...
        assertStatus(status, 200);
        assert((body as any).agent.driftScore === 0, 'driftScore should be 0 after baseline reset');
    });

    await test('GET /api/evals/alerts returns alerts list', async () => {
        const { status, body } = await get('/api/evals/alerts');
        assertStatus(status, 200);
        assert(Array.isArray((body as any).alerts), 'alerts should be array');
    });
...
async function testMarketing(): Promise<void> {
    ...
    await test('POST /api/marketing/execute runs a workflow', async () => {
        ...
        if (workflows?.length > 0) {
            ...
            assertStatus(status, 200);
            assert((body as any).execution, 'should return execution record');
        }
    });
